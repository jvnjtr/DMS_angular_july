...
@Component({
  selector: 'app-config-signature',
  templateUrl: './config-signature.component.html',
  styleUrls: ['./config-signature.component.scss']
})
export class ConfigSignatureComponent implements OnInit {
  ...
  utillist: any;
  messaageslist: any;
  jsonurl = "assets/js/_configs/signature.config.json";
  letterID: any = "";
  files_dropped: File[] = [];
  loading: any = false;
  ...
  createStamping() {
    ...
    let txtPosition = [{ 'x-axis': this.txtXaxis, 'y-axis': this.txtYaxis }];
     ...
     if (!this.vldChkLst.blankCheck(txtPage, this.commonserveice.langReplace(this.messaageslist.txtPage), 'txtPage')) {

    }else if (!this.vldChkLst.blankCheck(txtPosition, this.commonserveice.langReplace(this.messaageslist.signaturePosition), 'txtXaxis')) {

    }
    else {
      let metaparams = {
        "configId": this.configId,
        "signaturePosition": txtPosition,
        "signatureRotation":txtRotation,
        "signatureOpacity":txtOpacity,
        "signaturePage":txtPage,
      }
      this.loading = true;
      this.commonserveice.setSignatureConfiguration(metaparams).subscribe({
        next: (response) => {
