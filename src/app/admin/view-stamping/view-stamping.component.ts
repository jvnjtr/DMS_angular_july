...
@Component({
  selector: 'app-view-stamping',
  templateUrl: './view-stamping.component.html',
  styleUrls: ['./view-stamping.component.scss']
})
export class ViewStampingComponent implements OnInit {

  title: any;
  tablist: any;
  utillist: any;
  messaageslist: any;
  jsonurl = "assets/js/_configs/viewStamping.config.json";
  ...
  tablecollist = [
    ...
    { "name": "Created By", "cname": "createdBy" },
  ...
  getStampingConfig(configId:any) {
    ...
    this.loading = true;
    this.commonserveice.getStampConfigurationAll(metaparams).subscribe({
      next: (response) => {
  ...
  gotoEdit(id: any) {
    ...
    this.route.navigate(['/admin/editstampingconfig', encSchemeStr]);
  ...
  deletMeta(metaId: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.commonserveice.deleteStampingConfiguration(formParams).subscribe({
          next: (response) => {
            let respData = response.RESPONSE_DATA;
            let respToken = response.RESPONSE_TOKEN;
