...
export class PreviewFormApplyComponent implements OnInit {
  ...
  constructor(private router: ActivatedRoute, private WebCommonService: WebcommonservicesService, public encDec: EncrypyDecrpyService, private route: Router, private elRef: ElementRef,private sanitizer:DomSanitizer) { }
  ...
  ngOnInit(): void {
    ...
    this.historyId = (schemeArr[3] != '' && schemeArr[3] != undefined) ? schemeArr[3] : 0;
    ...
    this.WebCommonService.formToPdfgeneration(params).subscribe(resp => {
      ...
      if (respToken == verifyToken) {
        ...
        if (res.status == 200) {
          ...
          let formParms = fileName + ':' + fileName + ':' + fileType;
          let encSchemeStr = this.encDec.encText(formParms.toString());
          //
