...
@Component({
  selector: 'app-viewmanage-role',
  templateUrl: './viewmanage-role.component.html',
  styleUrls: ['./viewmanage-role.component.scss']
})
export class ViewmanageRoleComponent {
  public loading=false;

manage_rolelist : any;
 manage_rolelistPreview : any=[];
      ...
      viewAllData()
        {
            ...
            this.loading=true;
            this.commonService.viewAll(viewParams,'adminconsole/ManageRole/view').subscribe({
              next: (response) => {  
                ...
                res = JSON.parse(res.toString());
                  if(res.status==200){
                    this.loading=false;
                      ...
                      this.manage_rolelist=res.result;
                    }
                    else{
                      this.loading=false;
                      Swal.fire({
      ...
      res = JSON.parse(res.toString());
        if(res.status==200){
          this.loading=false;
            ...
            this.manage_rolelistPreview.push(res.result);
          }
          else{
            this.loading=false;
            Swal.fire({
...
*/
                 editData(id:any)
                 {
                    ...
                    this.route.navigate(['/adminconsole/manage-role',encSchemeStr]);
                 }       
...
*/
        deleteData(id:any)
        {
           ...
           Swal.fire({
            ...
            this.loading=true;
            this.commonService.deleteRecord(viewParams,'adminconsole/ManageRole/delete').subscribe({
              next: (response) => {  
                let respData = response.RESPONSE_DATA;
                let respToken = response.RESPONSE_TOKEN;
                let verifyToken = CryptoJS.HmacSHA256(respData, environment.apiHashingKey).toString();
