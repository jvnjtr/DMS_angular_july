...
export class ManageLinksComponent {
  ...
  */
  submitForm() {
    ...
    if ((errFlag == 0) && (!this.vldChkLst.selectDropdown(selLinkType, `Link Type`,'selLinkType'))) {
      errFlag = 1;
    }
    if ((errFlag == 0) && (!this.vldChkLst.blankCheck(txtLinkName, `Link name cannot be blank`, 'selLinkType'))) {
      errFlag = 1;
    }
    if ((errFlag == 0) && (!this.vldChkLst.blankCheck(txtURL, `Link URL cannot be blank`, 'txtLinkName'))) {
      errFlag = 1;
    }
